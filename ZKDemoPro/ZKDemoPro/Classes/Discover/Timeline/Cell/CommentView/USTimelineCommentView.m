...
@interface USTimelineCommentView() <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) UIImageView *bgImageView;
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSArray <USTimelineCommentCellLayout *> *dataSource;

@end

@implementation USTimelineCommentView
...
+ (CGFloat)calculateHeightWithCommentList:(NSArray<USComment *> *)commentList {
    ...
    CGFloat height = 0;
    for (USComment *comment in commentList) {
        USTimelineCommentCellLayout *layout = [[USTimelineCommentCellLayout alloc] initWithComment:comment];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    USTimelineCommentCell *cell = [USTimelineCommentCell cellWithTableView:tableView];
    [cell updateWithLayout:_dataSource[indexPath.item]];
    return cell;
}

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    USTimelineCommentCellLayout *layout = _dataSource[indexPath.item];
