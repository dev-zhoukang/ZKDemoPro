...
#import "USTimelineViewController.h"
#import "USTimelineCell.h"
#import "USTimelineCellLayout.h"
#import "MJRefresh.h"

@interface USTimelineViewController () <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, copy) NSMutableArray <USTimelineCellLayout *> *dataSource;
@property (nonatomic, assign) NSUInteger currentPage;

@end

@implementation USTimelineViewController
...
- (void)requestDataPullDown:(BOOL)isPullDown {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray <USTimeline *> *oriModels = [USTimeline modelListWithArray:dynamicArray];
        
        NSMutableArray *tempArray = [[NSMutableArray alloc] init];
        for (USTimeline *model in oriModels) {
            USTimelineCellLayout *layout = [[USTimelineCellLayout alloc] initWithTimelineModel:model];
...
- (void)setupUI {
    ...
    _tableView = [[UITableView alloc] initWithFrame:(CGRect){0,_topInset, SCREEN_WIDTH, SCREEN_HEIGHT-_topInset} style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    USTimelineCell *cell = [USTimelineCell cellWithTableView:tableView];
