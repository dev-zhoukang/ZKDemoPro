...
static id DBValueForKeyPath(id obj, NSString *keyPath) {
    ...
    for (NSString *key in keys) {
        if ([value isKindOfClass:[NSDictionary class]] || [value isKindOfClass:[DBObject class]] || [value isKindOfClass:[NSManagedObject class]]) {
...
+ (NSDictionary *)modelCustomPropertyMapper { return [self propertyMapper]; }
+ (NSDictionary *)modelContainerPropertyGenericClass { return [self propertyGenericClass]; }
...
+ (NSArray *)propertiesWithClass:(Class)cls {
    ...
    NSArray *properties = CFDictionaryGetValue(classCache, (__bridge const void *)(cls));
    
    dispatch_semaphore_signal(lock);
    if (!properties) {
        if ([cls isKindOfClass:[NSManagedObject class]]) {
            properties = [self propertiesOfClass:cls baseClass:[NSManagedObject class]];
        } else {
            properties = [self propertiesOfClass:cls baseClass:[NSObject class]];
        }
        
        if (properties) {
            dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
            CFDictionarySetValue(classCache, (__bridge const void *)(cls), (__bridge const void *)(properties));
            dispatch_semaphore_signal(lock);
        }
    }
    return properties;
...
{
    return [self dictionaryByRemovesKeysWithNullValues:YES];
}

- (NSDictionary *)dictionaryContainNull
{
    return [self dictionaryByRemovesKeysWithNullValues:NO];
}

- (NSDictionary *)dictionaryByRemovesKeysWithNullValues:(BOOL)removesKeysWithNullValues
{
